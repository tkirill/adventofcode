...
def run(monkeys: list[Monkey], globalmod: int):
    totals = [0] * len(monkeys)
    for i, monkey in enumerate(monkeys):
        totals[i] += len(monkey.items)
        for item in monkey.items:
            new_item = monkey.evalop(item) % globalmod
...
def runmany(monkeys: list[Monkey], common: int, n: int):
    totals = [0] * len(monkeys)
    for _ in range(n):
        tmp = run(monkeys, common)
...
totals = runmany(monkeys, globalmod, 20)
totals.sort()
print('Star 1:', totals[-1] * totals[-2])


monkeys, common = read_input()
totals = runmany(monkeys, common, 10*1000)
totals.sort()

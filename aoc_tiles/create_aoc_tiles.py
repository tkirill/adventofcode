...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    for year_dir in sorted(get_paths_matching_regex(AOC_DIR, YEAR_PATTERN), reverse=True):
        ...
        for solution in sorted(find_recursive_solution_files(year_dir)):
            if solution.suffix == '.txt' or not re.match(DAY_PATTERN, solution.name):
                continue
            day = find_first_number(solution.name)
            solutions = solution_paths_dict[year].setdefault(day, [])
            solutions.append(str(solution.relative_to(AOC_DIR).as_posix()))
...
def request_leaderboard(year: int) -> dict[int, DayScores]:
    ...
    with open(SESSION_COOKIE_PATH) as cookie_file:
        ...
        with open(leaderboard_path, "w", encoding='utf8') as file:
            file.write(data)
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: DayScores | None):
    ...
    day_graphic_path = day_graphic_path.relative_to(AOC_DIR).as_posix()

...
def step1(field: dict[Vec2, bool], cur: GridWalker) -> GridWalker:
    nxt_walker = cur.step()
...
def walk(
    ...
    step: Callable[[GridWalker], GridWalker],
) -> int:
    walker = GridWalker(Vec2(51, 1), screen.delta_down)
    for turn, dist in moves:
        ...
        for _ in range(dist):
            ...
            if not cell:
                break
            walker = nxt_walker
    return walker.pos.y * 1000 + walker.pos.x * 4 + DIRCOST[walker.velocity]


field, moves = readraw(strip=False).split("\n\n")

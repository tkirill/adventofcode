...
def biggest(bank: list[int]):
    for fd in range(9, 0, -1):
        fdi = colutils.index(bank, fd)
        if fdi == -1:
            continue
        for sd in range(9, 0, -1):
            sdi = colutils.index(bank, sd, fdi+1)
            if sdi != -1:
                return fd*10+sd
    ...
    for fd in range(9, 0, -1):
        ...
        for sd in range(9, 0, -1):
            ...
            for sd3 in range(9, 0, -1):
                sd3i = colutils.index(bank, sd3, sdi+1)
                if sd3i == -1:
                    continue
                for sd4 in range(9, 0, -1):
                    sd4i = colutils.index(bank, sd4, sd3i+1)
                    if sd4i == -1:
                        continue
                    for sd5 in range(9, 0, -1):
                        sd5i = colutils.index(bank, sd5, sd4i+1)
                        if sd5i == -1:
                            continue
                        for sd6 in range(9, 0, -1):
                            sd6i = colutils.index(bank, sd6, sd5i+1)
                            if sd6i == -1:
                                continue
                            for sd7 in range(9, 0, -1):
                                sd7i = colutils.index(bank, sd7, sd6i+1)
                                if sd7i == -1:
                                    continue
                                for sd8 in range(9, 0, -1):
                                    sd8i = colutils.index(bank, sd8, sd7i+1)
                                    if sd8i == -1:
                                        continue
                                    for sd9 in range(9, 0, -1):
                                        sd9i = colutils.index(bank, sd9, sd8i+1)
                                        if sd9i == -1:
                                            continue
                                        for sd10 in range(9, 0, -1):
                                            sd10i = colutils.index(bank, sd10, sd9i+1)
                                            if sd10i == -1:
                                                continue
                                            for sd11 in range(9, 0, -1):
                                                sd11i = colutils.index(bank, sd11, sd10i+1)
                                                if sd11i == -1:
                                                    continue
                                                for sd12 in range(9, 0, -1):
                                                    sd12i = colutils.index(bank, sd12, sd11i+1)
                                                    if sd12i != -1:

...
@cache
def arrangements(springs: str, groups: tuple[int]) -> int:
    if not springs:
        return not groups
    if not groups:
    ...
    if len(springs) < sum(groups) + len(groups) - 1:
        return 0
    if springs[0] == '.':
        return arrangements(springs.lstrip('.'), groups)
    total = 0
    if springs[0] == '?':
        total += arrangements(springs[1:], groups)
    if springs.find('.', 0, groups[0]) == -1 and (len(springs) == groups[0] or springs[groups[0]] != '#'):
        total += arrangements(springs[groups[0]+1:], groups[1:])
    return total


def star1():
    records = read(2023, 12, sep=r' |,')
    return sum(arrangements(springs, tuple(groups)) for springs, *groups in records)


def star2():
    records = [('?'.join([s]*5), tuple(g*5)) for s, *g in read(2023, 12, sep=r' |,')]
    return sum(arrangements(springs, groups) for springs, groups in records)
